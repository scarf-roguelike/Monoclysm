...
bool tileset::contain_tile_type(const std::string &id) const {
    const auto iter = tile_ids.find(id);
    if (iter != tile_ids.end()) {
        return true;
    }
    else {
        return false;
...
//const bool *tileset::try_get_tile_type(const std::string &id) 
//{
//
//}
...
tile_type nv_over_filter, nv_filter, shadow_filter;
static tile_type day_filter[4], dusk_filter[4], night_filter[4];
...
bool cata_tiles::init_filter()
{
    for (int i = 0; i < 4; i++) {
        if (tileset_ptr->contain_tile_type("day_filter" + std::to_string(i + 1))) {
            day_filter[i] = *tileset_ptr->find_tile_type("day_filter" + std::to_string(i + 1));
            auto picked = *day_filter[i].fg.pick(0);
            //auto &spritelist = *picked;
            //auto sprite_tex = tileset_ptr->get_tile(spritelist[0]);
            auto sprite_tex = tileset_ptr->get_tile(picked[0]);
            //sprite_tex->set_blendtexture(SDL_BLENDMODE_MOD);
            //sprite_tex->set_alphamode(50);
        }
        else {
            return false;
        };

        if (tileset_ptr->contain_tile_type("dusk_filter" + std::to_string(i + 1))) {
            dusk_filter[i] = *tileset_ptr->find_tile_type("dusk_filter" + std::to_string(i + 1));
            auto picked = *dusk_filter[i].fg.pick(0);
            auto sprite_tex = tileset_ptr->get_tile(picked[0]);
            //sprite_tex->set_blendtexture(SDL_BLENDMODE_MOD);
            //sprite_tex->set_alphamode(50);
        }
        ...
        if (tileset_ptr->contain_tile_type("night_filter" + std::to_string(i + 1))) {
            night_filter[i] = *tileset_ptr->find_tile_type("night_filter" + std::to_string(i + 1));
            auto picked = *night_filter[i].fg.pick(0);
            auto sprite_tex = tileset_ptr->get_tile(picked[0]);
            //sprite_tex->set_blendtexture(SDL_BLENDMODE_MOD);
            //sprite_tex->set_alphamode(50);
        }
    ...
    if (tileset_ptr->contain_tile_type("nv_filter")) {
        nv_filter = *tileset_ptr->find_tile_type("nv_filter");
    }
    else {
        return false;
    };

    if (tileset_ptr->contain_tile_type("nv_over_filter")) {
        nv_filter = *tileset_ptr->find_tile_type("nv_over_filter");
    }
    else {
        return false;
    };

    if (tileset_ptr->contain_tile_type("shadow_filter")) {
        nv_filter = *tileset_ptr->find_tile_type("shadow_filter");
    }
    else {
        return false;
    };
...
void cata_tiles::draw(int destx, int desty, const tripoint &center, int width, int height)
{
    ...
    for (int row = min_row; row < max_row; row++) {
        ...
        const std::array<decltype (&cata_tiles::draw_furniture), 8> drawing_layers = { {
            ...
            &cata_tiles::draw_critter_at,&cata_tiles::draw_filter,
        } };
...
// 필터 드로잉
bool cata_tiles::draw_filter(const tripoint &p, lit_level ll, int &height_3d) {

    if (use_filter) {

        int screen_x, screen_y;
        if (tile_iso) {
            screen_x = ((p.x - o_x) - (o_y - p.y) + screentile_width - 2) * tile_width / 2 + op_x;
            screen_y = ((p.y - o_y) - (p.x - o_x) - 4) * tile_width / 4 + screentile_height * tile_height / 2 + op_y;
        }
        else {
            screen_x = (p.x - o_x) * tile_width + op_x;
            screen_y = (p.y - o_y) * tile_height + op_y;
        }

        tile_type tile;// = *tileset_ptr->find_tile_type("day_filter2");

        std::string suffix = "";

        if (ll == LL_LOW || ll == LL_BRIGHT_ONLY) {
            suffix = "1";
        }
        else if (ll == LL_DIM) {
            suffix = "2";
        }
        else if (ll == LL_INDISTINCT) {
            suffix = "3";
        }
        else {
        ...
        else if (calendar::turn.is_dusk()) {
            tile = *tileset_ptr->find_tile_type("dusk_filter" + suffix);
        }
        else {
            tile = *tileset_ptr->find_tile_type("day_filter" + suffix);
        }

        auto picked = tile.fg.pick(0);
        if (!picked) {
            return true;
        }
        ...
        const texture *sprite_tex = tileset_ptr->get_tile(spritelist[0]);
        ...
        destination.x = screen_x + tile.offset.x * tile_width / tileset_ptr->get_tile_width();
        destination.y = screen_y + (tile.offset.y - height_3d) * tile_width / tileset_ptr->get_tile_width();
        destination.w = width * tile_width / tileset_ptr->get_tile_width();
        destination.h = height * tile_height / tileset_ptr->get_tile_height();

        sprite_tex->set_blendtexture(SDL_BLENDMODE_MOD);
        sprite_tex->render_copy(renderer, &destination);
        sprite_tex->set_blendtexture(SDL_BLENDMODE_BLEND);
    }
    return false;
}
