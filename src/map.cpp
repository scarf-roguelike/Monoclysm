...
lit_level map::apparent_light_at(const tripoint &p, const visibility_variables &cache) const {
    ...
    /*if (apparent_light > 1) {
        add_msg(m_debug, _("%d,%d : %f / %f"), p.x, p.y, apparent_light, cache.vision_threshold);
    }*/
    ...
    if (apparent_light > LIGHT_AMBIENT_LIT) {
        //add_msg(m_debug, _("%d,%d : lit + %f"), p.x, p.y, apparent_light);
        return LL_LIT;
    }
    if (apparent_light > LIGHT_AMBIENT_INDISTINCT) {
        //add_msg(m_debug, _("%d,%d : indistinct + %f"), p.x, p.y, apparent_light);
        return LL_INDISTINCT;
    }
    if (apparent_light > LIGHT_AMBIENT_DIM) {
        //add_msg(m_debug, _("%d,%d : dim + %f"), p.x, p.y, apparent_light);
        return LL_DIM;
    }

    if (apparent_light > cache.vision_threshold) {
        //add_msg(m_debug, _("%d,%d : low + %f"), p.x, p.y, apparent_light);
        return LL_LOW;
    }
    else {
        return LL_BLANK;
